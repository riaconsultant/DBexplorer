	...
	public StringBuilder startClassCreation(int fileType,
		...
		for (Map.Entry<String, Class<?>> field : fields.entrySet()) {
			if (field != null && field.getValue() != null
					...
					&& !field.getValue().getName().contains("lang") && !builder.toString().contains(field.getValue().getName())) {
		...
		if (dto instanceof RequestBean) {
			...
			for (MethodProperty mProperty : ((RequestBean) dto).getmProperty()) {
				...
				if (mProperty.getReturnType().equals("void")) {
					builder.append(reqBean.getBeanName()+"Dao ").append(reqBean.getBeanName().toLowerCase()+"dao = ").append("new "+reqBean.getBeanName()+"Dao();\n");
					builder.append(reqBean.getBeanName().toLowerCase()+"dao.").append(mProperty.getName()+"();\n");
					builder.append("return ;\n}\n");
				} else {
					builder.append(mProperty.getReturnType() + " obj = new "
							+ mProperty.getReturnType() + "();\n");
					builder.append(reqBean.getBeanName()+"Dao ").append(reqBean.getBeanName().toLowerCase()+"dao = ").append("new "+reqBean.getBeanName()+"Dao();\n");
					builder.append("obj = ").append(reqBean.getBeanName().toLowerCase()+"dao.").append(mProperty.getName()+"();\n");
